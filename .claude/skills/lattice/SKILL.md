...
# Non-interactive flag-based editing (scriptable, no $EDITOR needed)
lattice fact edit ADR-03 --title "Updated fact text" --reason "Clarified wording"

# Update tags (replaces existing)
lattice fact edit ADR-03 --tags "security,compliance" --reason "Recategorized"

# Update multiple fields at once
lattice fact edit ADR-03 --owner "new-team" --confidence Confirmed --reason "Ownership transfer"

# JSON output for scripting
lattice fact edit ADR-03 --title "New text" --json
```

**Available non-interactive flags**: `--title`/`--body` (fact text), `--tags` (comma-separated, replaces all), `--owner`, `--type`, `--confidence`, `--layer`, `--refs` (comma-separated, supports `CODE:edge_type`), `--projects` (comma-separated), `--review-by` (YYYY-MM-DD or empty to clear), `--reason` (changelog entry), `--json` (output updated fact as JSON).

If any flags are provided, the editor is skipped. Status changes are blocked — use `lattice fact promote` instead.
...
```bash
# Show all tags with usage counts and vocabulary categories
lattice tags

# JSON output
lattice tags --json

# Rebuild tags.yaml from current facts (regenerates the registry)
lattice tags --rebuild
```

Reads from `.lattice/tags.yaml` if it exists (fast path). Falls back to scanning all facts if the file is missing. Tags are categorized per DG-07 into: domain, concern, lifecycle, stakeholder, risk, or free. Free tags appearing in 3+ facts trigger a warning suggesting addition to the controlled vocabulary.

## Type Registry

```bash
# Show canonical type mappings (prefix -> type name)
lattice types

# JSON output
lattice types --json

# Audit facts for non-canonical types
lattice types --audit

# Audit output as JSON
lattice types --audit --json
```

Each code prefix has a canonical type (e.g., ADR -> "Architecture Decision Record", RISK -> "Risk Register Entry"). The `--audit` flag checks all facts and reports mismatches. Reads from `.lattice/types.yaml` if it exists, otherwise uses built-in defaults.
...
├── tags.yaml            # Tag registry (generated by `lattice tags --rebuild`)
├── types.yaml           # Type registry (prefix -> canonical type mapping)
